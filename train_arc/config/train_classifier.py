...
#          Fully dynamic to any taxonomy JSON configuration.
# ==============================================================================
...
def train_one_epoch(model, loader, criterion, optimizer, device):
    ...
    for images, genus_targets, species_targets in loader:
        images, genus_targets, species_targets = images.to(device), genus_targets.to(device), species_targets.to(device)
...
def evaluate(model, loader, criterion, device):
    model.eval()
    running_loss = 0.0
    correct_species, total_species = 0, 0
...
def main():
    ...
    num_genera, num_species = len(genus_id_map), len(species_id_map)

    print(f"\n================ TAXONOMY INITIALIZATION ================")
    print(f"Dataset Name    : {taxonomy_config.get('dataset_name', 'Taxa')}")
    ...
    model = HierarchicalTaxonomicClassifier(backbone_name=args.backbone, num_genera=num_genera, num_species=num_species).to(device)
